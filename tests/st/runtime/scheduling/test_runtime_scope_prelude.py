...
"""ST regression for runtime-scope prelude tensor slices.

The program writes disjoint windows of an ``Out`` tensor inside ``pl.at``
CORE_GROUP tasks, then reads the full parent tensor in a later loop. The
windowed producer's synthetic ``tensor.slice`` must be declared before the
generated ``PTO2_SCOPE`` so orchestration dependency analysis can still see the
parent/window relation outside the task scope.
"""
...
def _assert_window_view_is_prelude(code: str) -> None:
    view_match = re.search(r"Tensor\s+([A-Za-z_]\w*)\s*=\s*score_flat\.view\(", code)
    assert view_match is not None, code
    window_name = view_match.group(1)

    assert f"add_output({window_name})" in code, code
    assert "params_t1.add_input(score_flat)" in code, code

    loop_pos = code.index(f"for (int64_t c0 = 0; c0 < {_M}; c0 += {_SCORE_C})")
    view_pos = view_match.start()
    assert loop_pos < view_pos, code
    scope_pos = code.index("PTO2_SCOPE() {", view_pos)
    task_pos = code.index("// Task ", view_pos)
...
class TestRuntimeScopePreludeCodegen:
    ...
    def test_windowed_slice_is_runtime_scope_prelude(self, request, tmp_path, monkeypatch):
        ...
        assert "pl.tensor.slice(" in optimized and "score_flat" in optimized, optimized
        assert "produce__windowed" in optimized, optimized

        materialized = _latest_pass_dump(work_dir, "MaterializeRuntimeScopes")
        assert "pl.tensor.slice(" in materialized and "score_flat" in materialized, materialized

        code = _read_single(work_dir / "orchestration", "*.cpp")
        _assert_window_view_is_prelude(code)
