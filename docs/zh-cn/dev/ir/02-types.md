...
>
> **注意：** `TASK_ID` 是一个不透明的 64-bit handle（类型代码 `0x50`），表示 runtime 的 `PTO2TaskId`。它**不是**数值类型——上面没有任何算术运算。其值由 [`system.task_invalid`](05-operators.md)（"暂无 producer" 的哨兵）和 [`system.task_id_of(producer)`](05-operators.md)（kernel call LHS 背后的 task id）产生。大多数 `TASK_ID` 值由 [DeriveCallDirections pass](../passes/33-derive_call_directions.md) 的 manual scope 降级阶段合成；DSL 也暴露 `pl.task_id_of(result)`，允许用户命名某个 producer handle，并在 `with pl.manual_scope():` 内把这个 `pl.Scalar[pl.TASK_ID]` 变量传给 `deps=[...]`。codegen 把 `TASK_ID` 下沉为 `PTO2TaskId`。
